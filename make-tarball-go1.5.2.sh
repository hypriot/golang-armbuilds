...
GO_BOOTSTRAP_GOARMV=${GO_BOOTSTRAP_GOARMV:-64}
GO_BOOTSTRAP_VERSION=${GO_BOOTSTRAP_VERSION:-1.5.1}
rm -fr "$GOROOT_BOOTSTRAP"
mkdir -p "$GOROOT_BOOTSTRAP"
curl -sSL "https://github.com/hypriot/golang-armbuilds/releases/download/v${GO_BOOTSTRAP_VERSION}/go${GO_BOOTSTRAP_VERSION}.linux-arm${GO_BOOTSTRAP_GOARMV}.tar.gz" | tar xz -C "$GOROOT_BOOTSTRAP" --strip-components=1
...
cd ../..
tar --numeric-owner -czf "go${GO_VERSION}.linux-arm${GOARMV}.tar.gz" go
popd
mv "/usr/local/go${GO_VERSION}.linux-arm${GOARMV}.tar.gz" .
